...
class Category(BaseCategoryOrder):
    ...
    product_count: int = 0
    _unique_product_names: List[str] = []

    def __init__(self, name: str, description: str, products: list) -> None:
        super().__init__()

        # Проверка типов перед добавлением
    ...
    def add_product(self, product: Product) -> None:
        """Добавляет продукт в категорию и увеличивает счетчик уникальных товаров."""
        if not isinstance(product, Product):
            raise TypeError("Можно добавлять только продукты или их наследников!")

        self.__products.append(product)

        if product.name not in Category._unique_product_names:
            Category._unique_product_names.append(product.name)
    ...
    @property
    def total_quantity(self) -> int:
        """Возвращает общее количество всех штук продуктов в данной категории."""
